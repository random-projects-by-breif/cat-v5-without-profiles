...
local function downloadFile(path, func)
	if not isfile(path) or not shared.VapeDeveloper then
		local suc, res = pcall(function()
			return game:HttpGet('https://raw.githubusercontent.com/random-projects-by-breif/cat-v5-without-profiles/'..readfile('catrewrite/profiles/commit.txt')..'/'..select(1, path:gsub('catrewrite/', '')), true)
		end)
	...
	local success, result = callback(function(...)
		if isfile('catrewrite/games/'..game.PlaceId..'.lua') then
			...
			if not shared.VapeDeveloper then
				local suc, res = pcall(function()
					return game:HttpGet('https://raw.githubusercontent.com/random-projects-by-breif/cat-v5-without-profiles'..readfile('catrewrite/profiles/commit.txt')..'/games/'..game.PlaceId..'.lua', true)
				end)
